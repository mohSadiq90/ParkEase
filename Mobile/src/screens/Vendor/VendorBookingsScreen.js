...
import { 
    getVendorBookingsThunk, approveBookingThunk, rejectBookingThunk,
...
const VendorBookingsScreen = ({ navigation }) => {
    ...
    const handleApproveExtension = useCallback((id) => {
        Alert.alert('Approve Extension', 'Confirm approval of this time extension?', [
            { text: 'Cancel', style: 'cancel' },
            { text: 'Approve', onPress: () => dispatch(approveExtensionThunk(id)) },
        ]);
    }, [dispatch]);

    const handleRejectExtension = useCallback((id) => {
        Alert.alert('Decline Extension', 'Are you sure you want to decline this extension request?', [
            { text: 'Cancel', style: 'cancel' },
            { text: 'Decline', style: 'destructive', onPress: () => dispatch(rejectExtensionThunk({ id, reason: 'Declined by vendor' })) },
        ]);
    }, [dispatch]);
...
const styles = StyleSheet.create({
    ...
    extensionBox: { marginTop: spacing.md, padding: spacing.sm, backgroundColor: colors.warningSoft, borderRadius: spacing.radius.md, borderLeftWidth: 3, borderLeftColor: colors.warning },
    extensionTitle: { ...typography.bodySmall, color: colors.warningDark, fontWeight: '700' },
    extensionSubtitle: { ...typography.caption, color: colors.textSecondary, marginBottom: spacing.xs },
});
